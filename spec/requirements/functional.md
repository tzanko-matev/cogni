...
- Support `question_eval` tasks that evaluate Question Spec files.
- Support `cogni eval <questions_file> --agent <id>` for direct question evaluation.
- Evaluate each question with a single LLM run and compare the trailing `<answer>` output to correct answers.
...
- Capture per-question effort metrics for `question_eval` runs (tokens, wall time, tool calls).
...
- Missing question spec files or invalid answer formats must fail with clear errors.
